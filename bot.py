...
async def on_ready():
    ...
    print(app.user.id)
    print('oh yeah!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
    await app.change_presence(game=discord.Game(name="스캇"))
...
async def on_message(message):
    ...
    if message.content.startswith('야! 명령어'):
        await app.send_message(message.channel, '니가 알아서 찾으셈')

    if message.content.startswith('야! 색 종류'):
        await app.send_message(message.channel, '그렌 금갈치 아이보리 아쿠아 야수선배 여울 은갈치 초코 치마 핑크 하늘')

    if  message.content.startswith('야! 색 추가'):
        role = ""
        ...
        await app.send_message(message.channel, 'ㅇㅋ 바꿈')

    if  message.content.startswith('야! 색 제거'):
        role = ""
        rolename = message.content.split(" ")
        member = discord.utils.get(app.get_all_members(), id=message.author.id)
        for i in message.server.roles:
        ...
        await app.send_message(message.channel, 'ㅇㅋ 뺌')

    if message.content.startswith('야! 주사위'):
        await app.send_message(message.channel, '데구르르르르르르르')
        time.sleep(2)
        dice = 0
    ...
    if message.content.startswith('야! ㄱㄱ베스킨'):
        if os.path.isfile(message.server.id + "baskin.txt"):
            await app.send_message(message.channel, "이미 시작 했잖음;")
    ...
    if message.content.startswith('야! 베스킨'):
        ...
        if  int(team) >= 1 and int(team) <= 3:
            ...
            if  int(now_warn) <= 0:
                ...
                embed = discord.Embed(title="아웃!!!", description="하하하하하하하하하하하하하하하하하", color=0xFC67E0)
            ...
            await app.send_message(message.channel, "<@" + message.author.id + "> 할 줄 모르냐")
            

    if message.content.startswith('야! ㄱㄱ젠가'):
    ...
    if message.content.startswith('야! 젠가'):
    ...
    if message.content.startswith('야! 죽지마'):
        if message.content[6:].startswith(' <@'):
            ...
            if os.path.isfile(" v " + message.server.id + " _ " + vmention_id + ".txt"):
                ...
                f.write(now_warn)
                f.close()
                await app.send_message(message.channel, "<@" + message.author.id + "> 가 <@" + vmention_id + "> 에게 `" + ram + "`의 체력을 회복!!")
                time.sleep(0.5)
                if int(now_warn) > 150:
                    await app.send_message(message.channel, "<@" + vmention_id + "> 는 체력이 과다해서 폭팔했다!!!!!")
                    time.sleep(1)
                    role = ""
    ...
    if message.content.startswith('야! 나랑뜨자'):
        if message.content[7:].startswith(' <@'):
            ...
            if os.path.isfile(" v " + message.server.id + " _ " + vmention_id + ".txt"):
                ...
                f.write(now_warn)
                f.close()
                await app.send_message(message.channel, "<@" + message.author.id + "> 가 <@" + vmention_id + "> 에게 `" + ram + "`데미지를 입힘!")
                time.sleep(0.5)
                if int(now_warn) <= 0:
                    role = ""
                    member = discord.utils.get(app.get_all_members(), id=vmention_id)
                ...
                f.write("100")
                f.close()
                await app.send_message(message.channel,"<@" + vmention_id + "> 는 필드에 스폰되었다!!!!")
        else:
            await app.send_message(message.channel, "똑바로 써 병신아")


    if message.content.startswith('야! 똥코인'):
        if message.content[6:].startswith(' <@'):
            ...
            if os.path.isfile(message.server.id + " _ " + mention_id + ".txt"):
                ...
                f.write(now_warn)
                f.close()
                await app.send_message(message.channel, '두구두구두구두구두구')
                time.sleep(0.5)
                if int(ram) == 0:
                    ...
                    await app.send_message(message.channel, "동네 힘쌘<@" + message.author.id + "> 가 <@" + mention_id + "> 한테 똥 `" + ram + "`개를 돚거!\n")
                    time.sleep(0.5)
                    await app.send_message(message.channel, "<@" + mention_id + "> 는 벌써 `똥`이 `" + now_warn + "개`나 가지고 있네ㅋㅋㅋ")
            else:
                f = open(message.server.id + " _ " + mention_id + ".txt", 'w')
                f.write("1")
                f.close()
                await app.send_message(message.channel,"축하!!!\n<@" + message.author.id + "> 가 쑥쓰러워 하면서 <@" + mention_id + "> 한테 보물 1호를 선물!!\n<@" + mention_id + "> 는 처음으로 `똥`을 선물받음!")
        else:
            await app.send_message(message.channel, "똑바로 써 병신아")
    ...
    if message.content.startswith('야! 뿡져스텐'):
        ...
        if msg:
            if msg.content == '가위':
                await app.send_message(message.channel, 'https://i.imgur.com/0R5wEsV.png')
                time.sleep(1.0)
                role = ""
                member = discord.utils.get(app.get_all_members(), id=message.author.id)
                for i in message.server.roles:
                ...
                await app.send_message(message.channel, 'https://i.imgur.com/Ie11b5F.png')
                time.sleep(1.0)
                role = ""
                member = discord.utils.get(app.get_all_members(), id=message.author.id)
                for i in message.server.roles:
                ...
                await app.send_message(message.channel, 'https://i.imgur.com/63HTy9g.png')
                time.sleep(1.0)
                role = ""
                member = discord.utils.get(app.get_all_members(), id=message.author.id)
                for i in message.server.roles:
                ...
                await app.send_message(message.channel, 'https://i.imgur.com/4LVY9kE.png')
                time.sleep(1.0)
                role = ""
                member = discord.utils.get(app.get_all_members(), id=message.author.id)
                for i in message.server.roles:
